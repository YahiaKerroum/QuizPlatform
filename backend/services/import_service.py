...
async def process_import(db: AsyncPostgrestClient, rows: list[dict[str, Any]]) -> ImportOut:
    ...
    if modules_by_id:
        module_rows = [
            {"id": module_id, "display_name": module_name or module_id}
    ...
    all_quiz_ids = list(quizzes_by_id.keys())
    existing_quizzes_resp = await db.table("quizzes").select("id").in_("id", all_quiz_ids).execute()
    existing_quiz_ids = {row["id"] for row in (existing_quizzes_resp.data or [])}

    new_quiz_rows = [
        {"id": quiz_id, "display_name": topic, "module_id": quiz_modules_by_id.get(quiz_id)}
    ...
    quiz_ids_in_import = list({row["quiz_id"] for row in rows})
    existing_questions_resp = (
        await db.table("questions")
        .select("quiz_id,question_number")
        .in_("quiz_id", quiz_ids_in_import)
        .execute()
    )
