...
async def list_quizzes(db: AsyncPostgrestClient = Depends(get_db)) -> list[QuizSummaryOut]:
    return await list_quiz_summaries(db)


@router.get("/modules", response_model=list[ModuleWithQuizzesOut])
async def list_quiz_modules(db: AsyncPostgrestClient = Depends(get_db)) -> list[ModuleWithQuizzesOut]:
    return await list_modules_with_quizzes(db)


@router.get("/{quiz_id}", response_model=QuizDetailOut)
async def get_quiz(quiz_id: str, db: AsyncPostgrestClient = Depends(get_db)) -> QuizDetailOut:
